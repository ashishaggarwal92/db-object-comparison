...
@Component("test.job1.compare.table.tasklet")
@StepScope
public class TableObjectCompareTasklet implements Tasklet {

	private static final Logger LOGGER = LoggerFactory.getLogger(TableObjectCompareTasklet.class);
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		...
		if (enabled) {
			...
			if (CollectionUtils.isEmpty(objects1) && CollectionUtils.isEmpty(objects2)) {
				...
				List<String> matchedTables = new ArrayList<>(objects1);
				matchedTables.retainAll(objects2);
				LOGGER.info("{} : Names Matched in Both Schema are : {}", OBJECT_NAME, matchedTables);
				
				CustomCacheManager.matchedTableCache.addAll(matchedTables);
